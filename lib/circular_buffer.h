...
void circular_buffer<T,Extendable, Allocator>::Extend(){
    if(capacity_==0){
        ...
        for(size_type i = 0; i < size_; i++){
            std::allocator_traits<Allocator>::construct(alloc_, new_data+i,std::move(*it));
            it++;
        }
        it=begin();
        for(size_type i = 0; i < size_; i++){
...
circular_buffer<T, Extendable, Allocator>::circular_buffer(std::initializer_list<T> elements){
    ...
    for(auto it = elements.begin(); it != elements.end(); it++){
        std::allocator_traits<Allocator>::construct(alloc_, data_+start_, *it);
        start_++;
    }
    size_=elements.size();
    capacity_=size_;
...
template<typename T, bool Extendable, typename Allocator>
circular_buffer<T, Extendable, Allocator>::circular_buffer(const circular_buffer& other) : alloc_(other.alloc_){
...
template<typename T, bool Extendable, typename Allocator>
circular_buffer<T, Extendable, Allocator>::circular_buffer(circular_buffer&& other) : alloc_(std::move(other.alloc_)){
    if(other.alloc_==alloc_){
        ...
        other.data_=other.size_=other.capacity_=other.start_=other.end_=0;
        ...
        std::allocator_traits<Allocator>::deallocate(other.alloc_, other.data_, other.capacity_);
        other.data_=other.size_=other.capacity_=other.start_=other.end_=0;
...
template<typename T, bool Extendable, typename Allocator>
circular_buffer<T,Extendable, Allocator>::circular_buffer(circular_buffer&& other, const Allocator& alloc) : alloc_(alloc){
    if(other.alloc_==alloc_){
        data_=other.data_;
        capacity_=other.capacity_;
        size_=other.size_;
...
template<typename T, bool Extendable, typename Allocator>
circular_buffer<T,Extendable, Allocator>::circular_buffer(size_type n, const T& value){
    capacity_=n;
...
void circular_buffer<T, Extendable, Allocator>::push_back(U&& element){
    ...
    std::allocator_traits<Allocator>::construct(alloc_, data_+end_, std::forward<U>(element));
    size_++;
...
void circular_buffer<T, Extendable, Allocator>::push_front(U&& element){
    if constexpr (Extendable){
        ...
        if(size_ == capacity_){
            std::allocator_traits<Allocator>::destroy(alloc_, data_ + (start_ + size_ - 1) % capacity_);
            size_--;
        }
    }
    ...
    std::allocator_traits<Allocator>::construct(alloc_, data_+start_, std::forward<U>(element));
    size_++;
...
template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::front() const -> const_reference{
    if(size_ == 0){
...
template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::back() const -> const_reference{
    if(size_ == 0){
        throw std::out_of_range("0 elements in container");
    }
    return *(end()-1);
...
template<typename T, bool Extendable, typename Allocator>
void circular_buffer<T, Extendable, Allocator>::ShiftRight(iterator it, size_type n){
    if(n==0)return;
    if constexpr (Extendable){
    ...
    for(size_type to_move = size_; to_move >it.index_; to_move--){
        ...
        std::allocator_traits<Allocator>::construct(alloc_, data_ +insert_ind, std::move(data_[from_ind]));
...
template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::begin() const -> const_iterator{
...
template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::cbegin() const -> const_iterator{
    return const_iterator(this,0);
}
...
template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::rbegin() const -> const_reverse_iterator{
    return const_reverse_iterator(end());
}

template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::rend() const -> const_reverse_iterator{
    return const_reverse_iterator(begin());
}

template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::crbegin() const -> const_reverse_iterator{
    return const_reverse_iterator(cend());
}

template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::crend() const -> const_reverse_iterator{
...
auto circular_buffer<T, Extendable, Allocator>::insert(const_iterator it, ForwardIterator from, ForwardIterator to) -> iterator{
    ...
    for(;from!=to;from++){
        std::allocator_traits<Allocator>::construct(alloc_, data_ + (start_ +ind++)%capacity_, *from);
    }
    return iterator{this, first_inserted};
}

template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::insert(const_iterator it, const value_type& val) -> iterator{
    ...
    std::allocator_traits<Allocator>::construct(alloc_, data_ + (start_ +ind)%capacity_, val);
    return iterator{this, first_inserted};
    
}

template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::insert(const_iterator it, size_type cnt, const value_type& val) -> iterator{
...
template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::insert(const_iterator it, std::initializer_list<T> elements) -> iterator{
...
template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::erase(const_iterator it) -> iterator{

    if(size_ == 0 || it.index_ >= size_){
        throw std::out_of_range("erase(iterator) cant be executed. invalid iterator");
    }

    for(size_type ind = it.index_; ind<size_-1; ind++){
        data_[(start_+ind)%capacity_] = std::move(data_[(start_+ind+1)%capacity_]);
    }
    std::allocator_traits<Allocator>::destroy(alloc_, data_+(start_ + size_ - 1)%capacity_);
    size_--;
    end_=(start_+size_)%capacity_;
...
template<typename T, bool Extendable, typename Allocator>
auto circular_buffer<T, Extendable, Allocator>::erase(const_iterator start, const_iterator end) -> iterator{
    ...
    for(size_type ind = start.index_; ind<size_ - (end - start); ind++){
        data_[(start_+ind)%capacity_] = std::move(data_[(start_+ind+(end-start))%capacity_]);
    }
    for(size_type to_destroy = 1; to_destroy<=(end-start); to_destroy++){
        std::allocator_traits<Allocator>::destroy(alloc_, data_+(start_ + size_ - to_destroy)%capacity_);
    }
    size_-=(end-start);
    end_=(start_+size_)%capacity_;
...
template<typename T, bool Extendable, typename Allocator>
void circular_buffer<T, Extendable, Allocator>::assign(size_type n, const T& value){
    if constexpr(Extendable){
        while(capacity_ < n){
            Extend();
        }
    ...
    for(auto it = begin(); it!=end(); it++){
        std::allocator_traits<Allocator>::destroy(alloc_, data_+(start_+it.index_)%capacity_);
    }

    for(size_type i = 0; i < n; i++){
        std::allocator_traits<Allocator>::construct(alloc_, data_+(start_ + i)%capacity_, value);
    }
    size_=n;
    end_=(start_+size_)%capacity_;
}
...
template<typename T, bool Extendable, typename Allocator>
void circular_buffer<T, Extendable, Allocator>::assign(std::initializer_list<T> elements){
    ...
    for(auto it = elements.begin(); it!=elements.end() && ind < n; it++){
        std::allocator_traits<Allocator>::construct(alloc_, data_+(start_ + ind++)%capacity_, *it);
    }
    size_=n;
    end_=(start_+size_)%capacity_;
}
...
template<typename T, bool Extendable, typename Allocator>
template<typename ForwardIterator,bool>
void circular_buffer<T, Extendable, Allocator>::assign(ForwardIterator from, ForwardIterator to){
    size_type n = 0;
...
void circular_buffer<T, Extendable, Allocator>::resize(size_type n){
    ...
    if(size_ < n){
        for(size_type ind = size_; ind < n; ind++){
            std::allocator_traits<Allocator>::construct(alloc_, data_+(start_+ind)%capacity_, T{});
        }
    }
    else{
        for(size_type ind = n; ind < size_; ind++){
            std::allocator_traits<Allocator>::destroy(alloc_, data_+(start_+ind)%capacity_);
        }
    }
    size_=n;
    end_=(start_+size_)%capacity_;
...
template<typename T, bool Extendable, typename Allocator>
void circular_buffer<T, Extendable, Allocator>::resize(size_type n, const T& val){
    if constexpr (Extendable){
        while(n > capacity_){
            Extend();
        }
